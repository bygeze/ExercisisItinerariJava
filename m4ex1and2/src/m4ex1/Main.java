...
public class Main {
	...
	public static void main(String[] args) {
		...
		do {
			...
			try {
				...
				if(checkPlatoExists(input, 0, plats.length)) {
					System.out.println("Producto añadido: " + plats[input]);
					pedido.add(input);
				} else {
					System.out.println("El plato ingresado no existe, vuelva a intentarlo");
				}
	...
	static boolean checkPlatoExists(int i, int start, int end) {
		if(i > end - 1 || i < start) {
			return false;
		} else {
			return true;
